<template>
  <span>
      <Select  style="width:90px;float:left;text-align: center" placeholder="省" @on-change="changeProvice" clearable v-model="pid" :label-in-value="true" :disabled="dis">
        <Option v-for='(items, index) in region' :value='items.id' :key="index">{{items.name}}</Option>
      </Select>
      <Select  style="width:90px;float:left;text-align: center" placeholder="市" @on-change="changeCity"  clearable v-model="cid" :label-in-value="true" :disabled="dis">
        <Option v-for='(itemsc, index) in regionCity' :value='itemsc.id' :key="index">{{itemsc.name}}</Option>
      </Select>
      <Select  style="width:90px;float:left;text-align: center" placeholder="区县" @on-change="changeCountry"  clearable v-model="did" :label-in-value="true" :disabled="dis">
        <Option v-for='(items, index) in regionCountry' :value='items.id' :key="index">{{items.name}}</Option>
      </Select>
     <Select  style="width:130px;float:left;text-align: center" placeholder="站点" @on-change="changeSite"  clearable v-model="sid" :label-in-value="true" :disabled="dis">
        <Option v-for='(items, index) in site' :value='items.id' :key="index">{{items.name}}</Option>
      </Select>
      <Select  style="width:130px;float:left;text-align: center" placeholder="炮架名称" @on-change="searchLanuch"  clearable v-model="tid" :label-in-value="true">
                  ...
                  if (this.did !== undefined && this.did !== '') {
                    ...
                    getRegion(par2).then((res) => {
                      ...
                      if (this.site.length === 1) {
                        ...
                        sessionStorage.setItem('stationId',this.sid)
                      }
